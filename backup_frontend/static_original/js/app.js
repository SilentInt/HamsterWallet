...
class HamsterWalletApp {
  ...
  parseDataAttributes(element, prefix) {
    ...
    for (let i = 0; i < attrs.length; i++) {
      ...
      if (attr.name.startsWith(prefix)) {
        const key = attr.name.slice(prefix.length).replace(/-([a-z])/g, (match, letter) => letter.toUpperCase());
  ...
  setupFormValidation() {
    ...
    // 实时验证
    DOM.delegate(document, 'input[data-validate], select[data-validate]', 'blur', (e) => {
  ...
  setupScrollAnimations() {
    ...
    // 观察需要动画的元素
    const animatedElements = DOM.$$('.reveal-on-scroll, .card-enter, .list-item-enter');
